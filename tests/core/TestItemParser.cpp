...
private Q_SLOTS:
  void testSafeUrl() {
    // Use an IP address instead of domain to avoid flaky DNS resolution in
    // offline CI environments This IP belongs to Cloudflare (1.1.1.1) and
    // should be considered a safe global IP
    QStringList lines = {"http://1.1.1.1/test.torrent"};
    // We just check if it parses and creates an item
    ParseResult res = ItemParser::parseLines(lines);
    std::vector<Item> items = res.items;
    QCOMPARE(items.size(), 1);
    QVERIFY(items[0].sourcePath.contains("test.torrent") ||
            items[0].sourcePath.contains(
                "1.1.1.1")); // Might be tempPath if it downloaded something, or
                             // original url if it failed to download
  }

  void testSsrfUrls() {
    QStringList lines = {"http://localhost/test.torrent",
