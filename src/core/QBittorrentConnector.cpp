...
void QBittorrentConnector::onLoginReply() {
    ...
    if (reply->error() == QNetworkReply::NoError) {
        // Login successful. The cookie jar in QNetworkAccessManager should handle the session cookie automatically.
        // Proceed to dispatch pending item.
        if (m_isPending) {
        ...
        if (m_isPending) {
            emit dispatchFinished(m_pendingItem.id, false, "Login failed: " + reply->errorString());
            m_isPending = false;
        }
    }
    reply->deleteLater();
...
void QBittorrentConnector::performDispatch(const Item &item) {
    ...
    // We use QHttpMultiPart for both files and magnets because qBittorrent API accepts multipart/form-data
    ...
    // Since we are processing one at a time via `m_pendingItem` (for now), we can use that,
    // BUT since we might fire multiple requests if we improve the engine, let's attach the ID to the reply.
...
void QBittorrentConnector::onAddTorrentReply() {
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        if (response.toLower().contains("fail")) {
             emit dispatchFinished(itemId, false, "qBittorrent API returned failure: " + response);
        } else {
             emit dispatchFinished(itemId, true, "Dispatched successfully.");
        }
    } else {
        emit dispatchFinished(itemId, false, "Network error: " + reply->errorString());
    }
    reply->deleteLater();
}
