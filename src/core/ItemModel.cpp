...
QVariant ItemModel::data(const QModelIndex &index, int role) const {
    ...
    if (role == Qt::DisplayRole) {
        switch (index.column()) {
            ...
            case ColState: return item.stateToString();
            case ColSource: return item.sourcePath;
            case ColCreated: return item.createdTime.toString(Qt::ISODate);
...
QVariant ItemModel::headerData(int section, Qt::Orientation orientation, int role) const {
    ...
    switch (section) {
        ...
        case ColCreated: return "Created";
        default: return QVariant();
    }
}
