...
QString StorageManager::getBaseDir() const
{
    return m_baseDir;
}
QString StorageManager::getInboxDir() const
{
    return m_inboxDir;
}
QString StorageManager::getQueueDir() const
{
    return m_queueDir;
}
QString StorageManager::getDataDir() const
{
    return m_dataDir;
}
QString StorageManager::getManagedDir() const
{
    return m_managedDir;
}
...
bool StorageManager::deleteItem(const QString &id)
{
    ...
    QString cleanManagedDir = QDir::cleanPath(QDir(m_managedDir).absolutePath()) + "/";
    ...
    if (item.metadata.contains("managedFile")) {
        ...
        if (!managedPath.isEmpty() && QFile::exists(managedPath)) {
            QString cleanManagedPath = QDir::cleanPath(QFileInfo(managedPath).absoluteFilePath());
        ...
        QString cleanSourcePath = QDir::cleanPath(QFileInfo(item.sourcePath).absoluteFilePath());
...
std::vector<Item> StorageManager::loadItemsByStates(const QList<ItemState> &states)
{
    std::vector<Item> items;
    ...
    int totalSize = std::accumulate(states.begin(), states.end(), 0, [this](int sum, ItemState state) {
...
void StorageManager::onDirectoryChanged(const QString &path)
{
    if (path == m_inboxDir) {
        ...
        if (!newFiles.isEmpty()) {
            QThreadPool::globalInstance()->start([self = QPointer<StorageManager>(this), newFiles = newFiles]() {
...
bool StorageManager::moveToManaged(Item &item, bool deleteOriginal, bool skipSave)
{
