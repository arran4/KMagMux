...
Item Item::fromJson(const QJsonObject &json) {
    ...
    item.createdTime = QDateTime::fromString(json["createdTime"].toString(), Qt::ISODate);
    if (json.contains("scheduledTime") && !json["scheduledTime"].isNull()) {
        item.scheduledTime = QDateTime::fromString(json["scheduledTime"].toString(), Qt::ISODate);
    }
...
QString Item::stateToString() const {
    switch (state) {
        case ItemState::Unprocessed: return "Unprocessed";
        case ItemState::Queued: return "Queued";
        case ItemState::Scheduled: return "Scheduled";
        case ItemState::Held: return "Held";
        case ItemState::Dispatched: return "Dispatched";
        case ItemState::Failed: return "Failed";
        case ItemState::Archived: return "Archived";
        default: return "Unknown";
    }
}

ItemState Item::stringToState(const QString &s) {
    if (s == "Unprocessed") return ItemState::Unprocessed;
    if (s == "Queued") return ItemState::Queued;
    if (s == "Scheduled") return ItemState::Scheduled;
    if (s == "Held") return ItemState::Held;
    if (s == "Dispatched") return ItemState::Dispatched;
    if (s == "Failed") return ItemState::Failed;
    if (s == "Archived") return ItemState::Archived;
