    ...
    , m_paused(false)
{
    ...
    pluginPaths << QDir::cleanPath(appDir + "/" + QStringLiteral(KMAGMUX_REL_PLUGIN_DIR));
#endif
    ...
    for (const QString &path : pluginPaths) {
        ...
        for (QString fileName : pluginsDir.entryList(QDir::Files)) {
            ...
            bool isDev = versionStr.contains("development", Qt::CaseInsensitive) || versionStr.contains("dev", Qt::CaseInsensitive);
        ...
        if (plugin) {
            ...
            if (connector) {
                if (!m_connectors.contains(connector->getId())) {
                    qDebug() << "Loaded connector plugin:" << connector->getName() << "from" << info.filePath;
                    ...
                    connect(plugin, SIGNAL(dispatchFinished(QString, bool, QString)), this, SLOT(onDispatchFinished(QString, bool, QString)));
                } else {
            ...
            if (QLibrary::isLibrary(info.filePath)) {
                qWarning() << "Failed to load plugin" << info.filePath << ":" << pluginLoader.errorString();
            }
        }
    }
}
...
void Engine::onDispatchFinished(const QString &itemId, bool success, const QString &message, const QJsonObject &metadata)
{
