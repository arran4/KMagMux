...
TorBoxConnector::TorBoxConnector()
    : TorBoxConnector(nullptr)
{
}
...
QString TorBoxConnector::getId() const
{
    return "TorBox";
}

QString TorBoxConnector::getName() const
{
    return "TorBox";
}

bool TorBoxConnector::isEnabled() const
{
    return m_enabled;
}
...
void TorBoxConnector::dispatch(const Item &item)
{
    ...
    if (item.sourcePath.startsWith("magnet:")) {
        QHttpPart textPart;
        textPart.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=\"magnet\""));
    ...
    connect(reply, &QNetworkReply::finished, this, &TorBoxConnector::onAddTorrentReply);
}
...
bool TorBoxConnector::hasSettings() const
{
    return true;
}
...
bool TorBoxConnector::hasDebugMenu() const
{
    return true;
}
...
QList<HttpApiEndpoint> TorBoxConnector::getHttpApiEndpoints() const
{
    ...
    createTorrentMagnet.description = "Creates a new torrent transfer from a magnet link";
    createTorrentMagnet.method = "POST";
    createTorrentMagnet.url = "https://api.torbox.app/v1/api/torrents/createtorrent";
    ...
    createTorrentFile.description = "Creates a new torrent transfer from a .torrent file";
    createTorrentFile.method = "POST";
    createTorrentFile.url = "https://api.torbox.app/v1/api/torrents/createtorrent";
    createTorrentFile.headers.insert("Authorization", "Bearer ${API_TOKEN}");
    createTorrentFile.isMultipart = true;
    createTorrentFile.multipartParts.insert("file", "file:///path/to/test.torrent");
