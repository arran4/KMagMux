...
PutIoConnector::PutIoConnector()
    : PutIoConnector(nullptr)
{
}
...
QString PutIoConnector::getId() const
{
    return "PutIO";
}

QString PutIoConnector::getName() const
{
    return "Put.io";
}

bool PutIoConnector::isEnabled() const
{
    return m_enabled;
}
...
void PutIoConnector::dispatch(const Item &item)
{
    ...
    if (item.sourcePath.startsWith("magnet:")) {
        QHttpPart textPart;
        textPart.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=\"url\""));
    ...
    connect(reply, &QNetworkReply::finished, this, &PutIoConnector::onAddTorrentReply);
}
...
bool PutIoConnector::hasSettings() const
{
    return true;
}
...
bool PutIoConnector::hasDebugMenu() const
{
    return true;
}
...
QList<HttpApiEndpoint> PutIoConnector::getHttpApiEndpoints() const
{
    ...
    addTransferMagnet.headers.insert("Content-Type", "application/x-www-form-urlencoded");
    ...
    addTransferTorrent.description = "Creates a new transfer from a .torrent file";
    ...
    addTransferTorrent.multipartParts.insert("file", "file:///path/to/test.torrent");
