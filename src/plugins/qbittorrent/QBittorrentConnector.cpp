...
QBittorrentConnector::QBittorrentConnector()
    : QBittorrentConnector(nullptr)
{
}
...
QString QBittorrentConnector::getId() const
{
    return "qBittorrent";
}

QString QBittorrentConnector::getName() const
{
    return "qBittorrent (WebAPI)";
}

bool QBittorrentConnector::isEnabled() const
{
    return m_enabled;
}
...
void QBittorrentConnector::setCredentials(const QString &username, const QString &password)
{
    m_username = username;
    m_password = password;
}
...
void QBittorrentConnector::login()
{
    ...
    connect(reply, &QNetworkReply::finished, this, &QBittorrentConnector::onLoginReply);
}
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        if (response.trimmed() == "Fails.") {
            ...
            for (const Item &item : itemsToFail) {
                emit dispatchFinished(item.id, false, "Login failed: Invalid username or password." + apiCallLog);
            }
        } else {
        ...
        QString apiCallLog = reply->property("apiCallLog").toString();
        for (const Item &item : itemsToFail) {
            emit dispatchFinished(item.id, false, "Login failed: " + reply->errorString() + apiCallLog);
        }
    }
...
void QBittorrentConnector::performDispatch(const Item &item)
{
    ...
    if (item.sourcePath.startsWith("magnet:")) {
        QHttpPart textPart;
        textPart.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=\"urls\""));
    ...
    // 2. Torrent Files
        ...
        if (!file->open(QIODevice::ReadOnly)) {
            emit dispatchFinished(item.id, false, "Could not open torrent file: " + item.sourcePath);
        ...
        filePart.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=\"torrents\"; filename=\"" + filename + "\""));
        filePart.setBodyDevice(file);
        file->setParent(multiPart); // Valid trick: file is deleted when multiPart is deleted
    ...
    connect(reply, &QNetworkReply::finished, this, &QBittorrentConnector::onAddTorrentReply);
}
...
void QBittorrentConnector::onAddTorrentReply()
{
    ...
    if (statusCode == 401 || statusCode == 403) {
        ...
        if (response.toLower().contains("fail")) {
            emit dispatchFinished(itemId, false, "qBittorrent API returned failure: " + response + apiCallLog);
        } else {
        ...
        emit dispatchFinished(itemId, false, "Network error: " + reply->errorString() + apiCallLog);
    }

    reply->deleteLater();
}

bool QBittorrentConnector::hasSettings() const
{
    return true;
}
...
QWidget *QBittorrentConnector::createSettingsWidget(QWidget *parent)
{
    ...
    urlEdit->setText(settings.value("baseUrl", "http://localhost:8080").toString());
...
bool QBittorrentConnector::hasDebugMenu() const
{
    return true;
}
...
QList<HttpApiEndpoint> QBittorrentConnector::getHttpApiEndpoints() const
{
    ...
    addTorrentFile.multipartParts.insert("torrents", "file:///path/to/test.torrent");
