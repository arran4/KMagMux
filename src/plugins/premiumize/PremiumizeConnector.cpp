...
PremiumizeConnector::PremiumizeConnector()
    : PremiumizeConnector(nullptr)
{
}
...
QString PremiumizeConnector::getId() const
{
    return "Premiumize";
}

QString PremiumizeConnector::getName() const
{
    return "Premiumize.me";
}

bool PremiumizeConnector::isEnabled() const
{
    return m_enabled;
}
...
void PremiumizeConnector::dispatch(const Item &item)
{
    ...
    if (item.sourcePath.startsWith("magnet:")) {
        ...
        connect(reply, &QNetworkReply::finished, this, &PremiumizeConnector::onAddTorrentReply);
    } else {
        ...
        filePart.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=\"src\"; filename=\"" + filename + "\""));
...
bool PremiumizeConnector::hasSettings() const
{
    return true;
}
...
bool PremiumizeConnector::hasDebugMenu() const
{
    return true;
}
...
QList<HttpApiEndpoint> PremiumizeConnector::getHttpApiEndpoints() const
{
    ...
    accountInfo.url = "https://www.premiumize.me/api/account/info?apikey=${API_KEY}";
    ...
    transferList.url = "https://www.premiumize.me/api/transfer/list?apikey=${API_KEY}";
    endpoints.append(transferList);

    HttpApiEndpoint transferCreateMagnet;
    transferCreateMagnet.name = "Transfer Create (Magnet)";
    transferCreateMagnet.description = "Creates a new transfer from a magnet link";
    transferCreateMagnet.method = "POST";
    transferCreateMagnet.url = "https://www.premiumize.me/api/transfer/create";
    transferCreateMagnet.headers.insert("Content-Type", "application/x-www-form-urlencoded");
    ...
    transferCreateTorrent.description = "Creates a new transfer from a .torrent file";
    ...
    transferCreateTorrent.multipartParts.insert("src", "file:///path/to/test.torrent");
