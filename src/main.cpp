...
static void setupIpcHandler(QLocalServer &server, StorageManager &storage, MainWindow *window)
{
    ...
    QObject::connect(&server, &QLocalServer::newConnection, [&storage, &server, windowPtr]() {
        QLocalSocket *client = server.nextPendingConnection();
        QObject::connect(client, &QLocalSocket::readyRead, [&storage, client, windowPtr]() {
            ...
            for (int i = 0; i < passedArgs.size(); ++i) {
                ...
                Item newItem;
                newItem.id = QString::number(QDateTime::currentMSecsSinceEpoch()) + "_" + QString::number(i) + "_remote"; // To avoid collision
