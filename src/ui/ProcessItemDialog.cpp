...
ProcessItemDialog::ProcessItemDialog(const std::vector<Item> &items, const QStringList &connectors, QWidget *parent)
    : QDialog(parent)
    , m_items(items)
    , m_connectors(connectors)
{
    ...
    for (size_t i = 0; i < m_items.size(); ++i) {
        ...
        if (pathToCheck.startsWith("magnet:", Qt::CaseInsensitive) || pathToCheck.startsWith("http://", Qt::CaseInsensitive)
...
void ProcessItemDialog::setupUi()
{
    ...
    m_itemsTable->setHorizontalHeaderLabels({"Enable", "Delete file", "Delete when done", "Name", "Link"});
    ...
    connect(m_itemsTable, &QTableWidget::customContextMenuRequested, this, &ProcessItemDialog::onCustomContextMenuRequested);
    ...
    connect(m_stateCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &ProcessItemDialog::onStateChanged);
    ...
    connect(processBtn, &QPushButton::clicked, this, &ProcessItemDialog::onProcessClicked);
