    ...
    , m_currentTrackerIndex(0)
{
    ...
    connect(m_trackerClient, &TrackerClient::scrapeFinished, this, &TorrentInfoDialog::onScrapeFinished);
...
void TorrentInfoDialog::updateTrackerRow(int row, const TrackerStats &stats)
{
    if (stats.success) {
        ...
        m_logView->append(QString("Success for %1: Seeders=%2, Leechers=%3").arg(stats.trackerUrl).arg(stats.seeders).arg(stats.leechers));
    } else {
        ...
        m_logView->append(QString("Error for %1: %2").arg(stats.trackerUrl).arg(stats.errorString));
    }
}
