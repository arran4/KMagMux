    ...
    , m_currentReply(nullptr)
{
    ...
    connect(m_networkManager, &QNetworkAccessManager::finished, this, &ApiExplorerDialog::onNetworkReplyFinished);
    ...
    connect(m_networkManager, &QNetworkAccessManager::sslErrors, this, &ApiExplorerDialog::onSslErrors);
...
{
    setWindowTitle(QString("API Explorer - %1").arg(m_connector ? m_connector->getName() : "Unknown"));
    resize(900, 600);
    ...
    connect(m_endpointList, &QListWidget::currentRowChanged, this, &ApiExplorerDialog::onEndpointSelected);
    ...
    connect(sendButton, &QPushButton::clicked, this, &ApiExplorerDialog::onSendRequest);
    ...
    connect(addHeaderBtn, &QPushButton::clicked, this, &ApiExplorerDialog::onAddHeader);
    connect(removeHeaderBtn, &QPushButton::clicked, this, &ApiExplorerDialog::onRemoveHeader);
    ...
    connect(addMultiBtn, &QPushButton::clicked, this, &ApiExplorerDialog::onAddMultipart);
    connect(removeMultiBtn, &QPushButton::clicked, this, &ApiExplorerDialog::onRemoveMultipart);
    connect(browseMultiBtn, &QPushButton::clicked, this, &ApiExplorerDialog::onBrowseMultipartFile);
...
void ApiExplorerDialog::onBrowseMultipartFile()
{
    int row = m_multipartTable->currentRow();
    if (row < 0) {
        QMessageBox::information(this, "Select Row", "Please select a row first to set its file path.");
        return;
    }
...
void ApiExplorerDialog::onSendRequest()
{
    ...
    if (url.scheme() == "http") {
        qWarning() << "Security warning: Requesting an insecure HTTP URL in API Explorer.";
    }
    ...
    for (int i = 0; i < m_headersTable->rowCount(); ++i) {
        ...
        if (keyItem && valItem && !keyItem->text().isEmpty()) {
            request.setRawHeader(applySubstitutions(keyItem->text()).toUtf8(), applySubstitutions(valItem->text()).toUtf8());
        }
    }
    ...
    if (m_isMultipartCurrent) {
            ...
            if (keyItem && valItem && !keyItem->text().isEmpty()) {
                ...
                if (val.startsWith("file://")) {
                    ...
                    if (file->open(QIODevice::ReadOnly)) {
                        ...
                        part.setHeader(QNetworkRequest::ContentDispositionHeader, QVariant("form-data; name=\"" + key + "\"; filename=\"" + filename + "\""));
                        part.setBodyDevice(file);
                        file->setParent(multiPart); // File will be deleted with multipart
                    } else {
                        QMessageBox::warning(this, "File Error", "Failed to open file: " + filePath);
                        file->deleteLater();
        ...
        if (method == "POST") {
            ...
            m_currentReply = m_networkManager->sendCustomRequest(request, method.toUtf8(), multiPart);
        }
        ...
        if (method == "GET") {
            ...
            m_currentReply = m_networkManager->sendCustomRequest(request, method.toUtf8(), body);
        }
    }
}
...
void ApiExplorerDialog::onNetworkReplyFinished()
{
    ...
    m_statusLabel->setText(QString("Status: %1 %2").arg(statusCode).arg(reply->error() != QNetworkReply::NoError ? reply->errorString() : "OK"));
...
void ApiExplorerDialog::onSslErrors(QNetworkReply *reply, const QList<QSslError> &errors)
{
    QString errorString;
    ...
    QMessageBox::critical(this, "SSL Error", QString("SSL Verification Failed:\n%1").arg(errorString));
