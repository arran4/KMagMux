...
AddItemDialog::AddItemDialog(const std::vector<Item> &items, const QStringList &connectors, QWidget *parent)
    : QDialog(parent)
    , m_items(items)
{
    setupUi();
...
void AddItemDialog::setupUi()
{
    ...
    m_itemsTable->setHorizontalHeaderLabels({"Enable", "Delete file", "Name", "Link"});
    ...
    connect(m_itemsTable, &QTableWidget::customContextMenuRequested, this, &AddItemDialog::onCustomContextMenuRequested);
    ...
    connect(processBtn, &QPushButton::clicked, this, &AddItemDialog::onProcessClicked);
