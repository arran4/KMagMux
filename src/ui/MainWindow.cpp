...
MainWindow::MainWindow(StorageManager *storage, QWidget *parent)
    ...
    , m_engine(nullptr)
    , m_tabWidget(nullptr)
    , m_unprocessedModel(nullptr)
    , m_queueModel(nullptr)
    , m_doneModel(nullptr)
    , m_archiveModel(nullptr)
    , m_errorModel(nullptr)
    , m_unprocessedProxy(nullptr)
    , m_queueProxy(nullptr)
    , m_doneProxy(nullptr)
    , m_archiveProxy(nullptr)
    , m_errorProxy(nullptr)
    , m_unprocessedView(nullptr)
    , m_queueView(nullptr)
    , m_doneView(nullptr)
    , m_archiveView(nullptr)
    , m_errorView(nullptr)
    , m_toggleProcessingAction(nullptr)
    , m_selectAllAction(nullptr)
    , m_processAction(nullptr)
    , m_processAllAction(nullptr)
    , m_unprocessAction(nullptr)
    , m_dismissAction(nullptr)
    , m_archiveAction(nullptr)
    , m_archiveAllAction(nullptr)
    , m_deleteAction(nullptr)
    , m_infoAction(nullptr)
    , m_rawResultsAction(nullptr)
    , m_trayIcon(nullptr)
    ...
    , m_forceQuit(false)
{
    ...
    connect(m_storage, &StorageManager::itemsUpdated, this, &MainWindow::onItemsUpdated);
    connect(m_storage, &StorageManager::itemDeleted, this, &MainWindow::onItemDeleted);
    connect(m_storage, &StorageManager::itemsDeleted, this, &MainWindow::onItemsDeleted);
}
...
void MainWindow::dropEvent(QDropEvent *event)
{
    ...
    if (!lines.isEmpty() || !filesToRead.isEmpty()) {
        ...
        connect(watcher, &QFutureWatcher<std::vector<Item>>::finished, this, [this, watcher]() {
        ...
        QFuture<std::vector<Item>> future = QtConcurrent::run([lines, filesToRead]() -> std::vector<Item> {
...
void MainWindow::setupActionsAndMenus()
{
    ...
    connect(m_processAction, &QAction::triggered, this, &MainWindow::onProcessItem);
    ...
    connect(m_unprocessAction, &QAction::triggered, this, [this]() {
        onItemAction(ItemState::Unprocessed);
    });
    ...
    connect(m_dismissAction, &QAction::triggered, this, [this]() {
        onItemAction(ItemState::Archived);
    });
    ...
    connect(m_archiveAction, &QAction::triggered, this, [this]() {
        onItemAction(ItemState::Archived);
    });
    ...
    QAction *openCacheAction = new QAction(QIcon::fromTheme("folder-open"), tr("Open &Cache directory"), this);
...
void MainWindow::setupTabs()
{
    ...
    auto setupView = [this](QTableView *view, ItemModel *model, ItemFilterProxyModel *proxy, const QString &title) {
        ...
        connect(view->selectionModel(), &QItemSelectionModel::selectionChanged, this, &MainWindow::updateActionsState);
    ...
    connect(m_queueView, &QTableView::doubleClicked, this, &MainWindow::onProcessItem);
    ...
    connect(m_doneView, &QTableView::doubleClicked, this, &MainWindow::onProcessItem);
    ...
    connect(m_archiveView, &QTableView::doubleClicked, this, &MainWindow::onProcessItem);
    ...
    connect(m_errorView, &QTableView::doubleClicked, this, &MainWindow::onProcessItem);

    connect(m_tabWidget, &QTabWidget::currentChanged, this, &MainWindow::updateActionsState);
    updateActionsState();
}
...
void MainWindow::setupSystemTray()
{
    ...
    connect(m_showHideAction, &QAction::triggered, this, &MainWindow::toggleShowHide);
...
void MainWindow::onItemsUpdated()
{
    loadData();
}

void MainWindow::onItemDeleted(const QString &id)
{
    loadData();
}

void MainWindow::onItemsDeleted(const std::vector<QString> &ids)
{
    loadData();
}
...
void MainWindow::onAddItems()
{
    ...
    connect(browseBtn, &QPushButton::clicked, this, [textEdit, this]() {
        ...
        if (!files.isEmpty()) {
            ...
            currentText = std::accumulate(files.begin(), files.end(), currentText, [](const QString &a, const QString &b) {
                return a + "file://" + b + "\n";
            });
    ...
    connect(openListBtn, &QPushButton::clicked, this, [&dialog, textEdit, this]() {
        QDialog openDialog(&dialog);
        openDialog.setWindowTitle(tr("Open List/HTML"));
        QVBoxLayout *olLayout = new QVBoxLayout(&openDialog);

        QLabel *olLabel = new QLabel(tr("Enter URLs or local file paths to text lists or HTML files:"), &openDialog);
        ...
        QCheckBox *extractMagnetsCb = new QCheckBox(tr("Extract magnet links"), &openDialog);
        extractMagnetsCb->setChecked(true);
        olLayout->addWidget(extractMagnetsCb);

        QCheckBox *extractTorrentsCb = new QCheckBox(tr("Extract torrent links"), &openDialog);
        extractTorrentsCb->setChecked(true);
        olLayout->addWidget(extractTorrentsCb);

        QDialogButtonBox *olButtonBox = new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel, &openDialog);
        olLayout->addWidget(olButtonBox);

        connect(olButtonBox, &QDialogButtonBox::accepted, &openDialog, &QDialog::accept);
        connect(olButtonBox, &QDialogButtonBox::rejected, &openDialog, &QDialog::reject);

        if (openDialog.exec() == QDialog::Accepted) {
            QStringList lines = olTextEdit->toPlainText().split('\n', Qt::SkipEmptyParts);
            if (!lines.isEmpty()) {
                LinkExtractorDialog extractor(lines, extractMagnetsCb->isChecked(), extractTorrentsCb->isChecked(), &dialog);
...
void MainWindow::processAddedLines(const QStringList &lines)
{
    ...
    QFuture<std::vector<Item>> future = QtConcurrent::run([lines]() -> std::vector<Item> {
...
void MainWindow::onProcessItem()
{
    ...
    std::transform(selection.begin(), selection.end(), std::back_inserter(selectedItems), [model, proxy](const QModelIndex &index) {
        QModelIndex sourceIndex = proxy->mapToSource(index);
    ...
    for (Item &updatedItem : updatedItems) {
        ...
        if (updatedItem.metadata.value("delete_source_file").toBool(false)) {
            ...
            if (!m_storage->moveToManaged(updatedItem, true, true)) {
                QMessageBox::warning(this, "Error", QString("Failed to move original file to managed storage: %1").arg(updatedItem.sourcePath));
...
void MainWindow::setupPluginMenus(QMenu *helpMenu)
{
    ...
    for (const QString &connectorId : m_engine->getAvailableConnectors()) {
        ...
        if (connector && connector->hasDebugMenu()) {
            QMenu *pluginMenu = helpMenu->addMenu(QString("%1 Debug").arg(connector->getName()));
            ...
            if (!endpoints.isEmpty()) {
                ...
                connect(apiExplorerAction, &QAction::triggered, this, [this, connector]() {
                    onOpenApiExplorer(connector);
                });
            }
        }
    }
