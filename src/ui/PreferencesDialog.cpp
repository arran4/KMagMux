    ...
    , m_engine(engine)
{
    ...
    connect(m_categoriesList, &QListWidget::currentItemChanged, this, &PreferencesDialog::changePage);
    ...
    connect(m_buttonBox->button(QDialogButtonBox::Apply), &QPushButton::clicked, this, [this]() {
        ...
        settings.setValue("allowPlaintextStorage", m_allowPlaintextStorageCb->isChecked());
    });
...
void PreferencesDialog::createGeneralPage()
{
    ...
    QSettings settings;
    m_closeToTrayCb = new QCheckBox(tr("When window is closed, minimize to tray"), page);
    m_closeToTrayCb->setChecked(settings.value("closeToTray", false).toBool());
    layout->addWidget(m_closeToTrayCb);

    m_minimizeToTrayCb = new QCheckBox(tr("When window is minimized, minimize to tray"), page);
    m_minimizeToTrayCb->setChecked(settings.value("minimizeToTray", false).toBool());
    ...
    m_allowPlaintextStorageCb = new QCheckBox(tr("Allow plaintext password and API key storage (fallback "
                                                 "if keychain fails)"),
                                              page);
    m_allowPlaintextStorageCb->setChecked(settings.value("allowPlaintextStorage", false).toBool());
    ...
    if (m_engine) {
        ...
        if (connectors.isEmpty()) {
            QLabel *noPluginsLabel =
                new QLabel(tr("No plugins found. Ensure plugins are compiled into the 'plugins' "
                              "directory.\nLooking in: ")
                               + QCoreApplication::applicationDirPath() + "/plugins and " + QCoreApplication::applicationDirPath() + "/../plugins",
                           scrollContent);
            ...
            for (const QString &id : connectors) {
                ...
                if (connector) {
                    QGroupBox *groupBox = new QGroupBox(connector->getName(), scrollContent);
                    ...
                    if (connector->hasSettings()) {
                        ...
                        if (settingsWidget) {
                            ...
                            // saveSettings.
                            if (m_buttonBox) {
                                connect(m_buttonBox, &QDialogButtonBox::accepted, this, [connector, settingsWidget]() {
                                    connector->saveSettings(settingsWidget);
                                });
                            }
                        } else {
                            groupLayout->addWidget(new QLabel(tr("Settings widget creation failed.")));
                        }
                    } else {
