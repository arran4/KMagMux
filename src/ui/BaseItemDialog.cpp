...
BaseItemDialog::BaseItemDialog(const std::vector<Item> &items, QWidget *parent)
    : QDialog(parent), m_items(items) {}

std::vector<Item> BaseItemDialog::getItems() const { return m_items; }

void BaseItemDialog::setupTable() {
  ...
  connect(m_itemsTable, &QTableWidget::customContextMenuRequested, this,
          &BaseItemDialog::onCustomContextMenuRequested);
}

void BaseItemDialog::populateTable() {
...
void BaseItemDialog::onCustomContextMenuRequested(const QPoint &pos) {
